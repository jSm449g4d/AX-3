            ...
            with tqdm(total=data.shape[0]) as pbar:
                while pbar.n+batch<pbar.total:
                    ...
                    with tf.GradientTape() as tape:
                        gen=self.gen(label)
                        grad=self.dis(gen)
                        grad=keras.losses.binary_crossentropy(ones,grad)
                        grad=tf.reduce_mean(grad) 
                        ####mode seeking (tentative implement)###
                        roll_shift=random.randint(1,batch-1)
                        Lms=tf.reduce_mean(tf.abs(gen-tf.roll(gen,roll_shift,axis=0)),[1,2,3])
                        Lms/=tf.reduce_mean(tf.abs(label-tf.roll(label,roll_shift,axis=0)),[1])
                        Lms=tf.math.reciprocal(Lms+np.full(batch,1e-5))
                        grad+=tf.reduce_mean(Lms) 
                        ####mode seeking (tentative implement)###
                        grad=tape.gradient(grad,self.gen.trainable_variables) 
                        grad,_ = tf.clip_by_global_norm(grad, 15)
